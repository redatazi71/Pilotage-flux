"""Étude comparative : OF / OF+EVENT / FLUX+EVENT ± BCE.

Mesure sur scénario stress (60j, hazards multiples) :
  - QCDS complet (Q compliance, Coût/u, Dispo SO, S nervousness)
  - OTIF (Q × D)
  - WIP moyen + σ
  - Rupture : % de SOs rejetées (livraison au-delà due_date + tolerance)
  - Recovery : temps de retour du WIP à la bande nominale après pic

Configurations testées :
  1. OF pur
  2. OF + EVENT (boucle événementielle V3)
  3. FLUX + EVENT (=DOCTRINE_EVENT — smoothing + event sourcing)
  4. OF + EVENT + BCE (=DOCTRINE_OF_EVENT_BCE — boucle cybernétique
     étendue avec zone négociable enrichie V13.D+E+F+K)
  5. FLUX + EVENT + BCE (=DOCTRINE_EVENT_BCE)

Sortie :
  - docs/of_flux_event_bce_study_runs.csv (1 ligne par run)
  - docs/of_flux_event_bce_summary.json (agrégation par doctrine)
  - docs/of_flux_event_bce_report.md (tableau comparatif)

Usage : python docs/build_of_flux_event_bce_study.py [--seeds 5]
"""
...
RUNS_CSV = HERE / "of_flux_event_bce_study_runs.csv"
SUMMARY_JSON = HERE / "of_flux_event_bce_summary.json"
...
def _run_one(scen, doctrine, work, fix_dir, config_tag) -> dict:
    ...
    try:
        ...
        # Rupture % = SOs rejetées / SOs totaux
        rupture_pct = (
            k.so_rejected / k.so_total if k.so_total > 0 else 0.0
        )
        # Recovery : après le 1er hazard (jour du 1er hazard scenario)
...
def _agg_by_config(runs: list[dict]) -> dict:
    """Agrège par tag config → moyennes des métriques."""
...
def _write_report(agg: dict) -> None:
    ...
    lines = [
        "# Étude comparative OF / OF+EVENT / FLUX+EVENT ± BCE",
        "",
        f"Horizon {HORIZON_DAYS}j, {N_HAZARDS} hazards, "
        f"{N_SEEDS_DEFAULT} seeds par configuration.",
        "",
        "## Tableau QCDS + OTIF + WIP + Rupture + Recovery",
        "",
        "| Configuration | OTIF | Q | D | €/u | WIP moy | WIP σ | Nervosité | Rupture % | Recovery j |",
        "|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|",
    ]
    ...
    lines.append("")
    lines.append("### FLUX+EVENT vs OF vs OF+EVENT (baseline)")
    lines.append("")
...
def main() -> int:
    ...
    seeds = list(range(700, 700 + args.seeds))
    n_total = len(seeds) * len(CONFIGS)
    print(f"=== Étude OF / OF+EVENT / FLUX+EVENT ± BCE ===")
    print(f"Horizon {HORIZON_DAYS}j × {N_HAZARDS} hazards × "
          f"{len(CONFIGS)} configs × {len(seeds)} seeds = "
          f"{n_total} runs")

    all_runs: list[dict] = []
    with TemporaryDirectory(prefix="of_flux_bce_") as tmp:
        work = Path(tmp)
        fix_dir = work / "fix"
